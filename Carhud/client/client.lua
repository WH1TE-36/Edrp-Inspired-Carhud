...
RegisterNetEvent('Carhud:big-map')
AddEventHandler('Carhud:big-map', function(bool)
    if Show then
        if bool then
...
RegisterNetEvent('Carhud:client:caps-lock')
AddEventHandler('Carhud:client:caps-lock', function(status)
...
RegisterCommand('hud', function()
    TriggerEvent('Carhud:open-hud')
    ESX.TriggerServerCallback('Carhud:getAccounts', function(accounts)
        cashAmount = accounts.cash
        bankAmount = accounts.bank
    end)
end)

RegisterNetEvent('Carhud:open-hud')
AddEventHandler('Carhud:open-hud', function()
    if not Show then
        PlayerData = ESX.GetPlayerData()
        TriggerEvent("Carhud:load-data")
        SendNUIMessage({action = 'showui'})
        UIStuff()
        isLoggedIn = true
        Show = true
...
RegisterNetEvent('Carhud:load-data')
AddEventHandler('Carhud:load-data', function(health, armor)
