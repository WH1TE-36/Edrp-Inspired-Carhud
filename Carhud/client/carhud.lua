...
RegisterCommand("cruise", function(source, args)
    ...
    if driverSeat and tonumber(args[1]) > 0 then
        -- if not IsVehicleTyreBurst(vehicle, 0) and not IsVehicleTyreBurst(vehicle, 1) and not IsVehicleTyreBurst(vehicle, 4) and not IsVehicleTyreBurst(vehicle, 5) then 
            ...
            exports['mythic_notify']:DoHudText('success', 'Hız sablitleyici açık.')
        -- end
    end
end)

RegisterCommand("cruiseoff", function(source, args)
    if args[1] == nil then args[1] = 1 end
    if driverSeat and tonumber(args[1]) > 0 then
        -- if not IsVehicleTyreBurst(vehicle, 0) and not IsVehicleTyreBurst(vehicle, 1) and not IsVehicleTyreBurst(vehicle, 4) and not IsVehicleTyreBurst(vehicle, 5) then 
            cruiseIsOn = fasle
            cruiseSpeed = (tonumber(args[1]) / 3.6) / 1.3
            exports['mythic_notify']:DoHudText('error', 'Hız sablitleyici kapalı.')
        -- end
    end
end)

Citizen.CreateThread(function()
    TriggerEvent('chat:addSuggestion', '/cruise', 'Aracın hızını sabitlemek için bu komutu kullanabilirsin.', {{ name="Sabitlenecek Hız", help="35-999"}})
    TriggerEvent('chat:addSuggestion', '/cruiseoff', 'Hız sabitleyici kapatmak için.')
end)

RegisterNetEvent("Carhud:car:eject-other-player-car-client")
AddEventHandler("Carhud:car:eject-other-player-car-client", function(velocity)
    ejectPlayer()
end)
                        ...
                        if vehIsMovingFwd and vehAcc then
                            if prevSpeed*3.6 > triggerSpeed then
                                ...
                                if #seatPlayerId > 0 then
                                    TriggerServerEvent("Carhud:car:eject-other-player-car", seatPlayerId, prevVelocity)
                                end
                                ejectPlayer()
